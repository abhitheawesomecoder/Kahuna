...
typedef enum {
    KAHLocationServicesAccuracySettingAlways     = 0 << 1,    // This monitors for regions (circular) to find if a user enters or exits a region.
    KAHLocationServicesAccuracySettingAppOpen     = 1 << 1    // This monitors for regions (circular) to find if a user enters or exits a region.
} KAHLocationServicesAccuracySetting;
...
/*!
 ...
 @abstract
 This API is used to force the SDK event traffic to your Sandbox development namespace.
 
 @discussion
 Use this API for testing purposes only. If part of the QA process involves a signed build but still want traffic to Sandbox, this API will force the staging
 app to bypass normal auto environment detection.
 <br><br>
 <b><font color="red">IMPORTANT !!</font></b> Do not ship your application to the app store with Force Sandbox turned on. <br>
 
 @param forceSandbox                True/False to turn on/off force sandbox mode.
 */
+ (void) setForceSandbox : (Boolean) forceSandbox;
...
+ (void) enableLocationServices:(KAHLocationServicesFeatures) features withReason:(NSString*) reason locationAccuracy:(KAHLocationServicesAccuracySetting)locationAccuracy;
